...
public class AssertionResolver {
    ...
    public static ActionAssertion actionAssertion(InstrumentationTestCase testCase,
        ...
        if (testCase instanceof CalculonUnitTest<?>) {
            return new UnitTestActionAssertion(testCase, activity, action, runOnMainThread);
        } else if (testCase instanceof CalculonStoryTest<?>) {
            return new StoryTestActionAssertion(testCase, activity, action, runOnMainThread);
        } else {
